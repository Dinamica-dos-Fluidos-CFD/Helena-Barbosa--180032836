...
No caso estudado, a perda de carga aferida na seção da tubulação apresentou-se divergente do valor calculado. O cálculo adotou o menor valor possível para o fator de atrito ao considerar a superfície do tubo como "lisa". Porém, o valor teórico da perda de carga foi maior do que o medido no local. Sendo assim, uma possível causa para essa anormalidade seria a existência de uma deformação no trecho da tubulação, comprometendo sua uniformidade interna. A partir disto, seria possível ocorrer mudanças no comportamento do escoamento, devido a divergência entre a geometria adotada e a situação real. Por fim, conforme o que foi proposto, o objetivo do projeto foi concluído.
